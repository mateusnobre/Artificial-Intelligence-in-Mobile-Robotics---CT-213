...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        for i in range(num_neighbors):
            neighbors_list.append((theta[0]+delta*sin(2*pi*i/num_neighbors),theta[1]+delta*cos(2*pi*i/num_neighbors)))
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        """
        # Todo: Implement
        i = random.uniform(-pi,pi)
        return (theta[0]+delta*sin(i),theta[1]+delta*cos(i))
    def schedule(i):
        """
...
data = np.genfromtxt('data.txt') # adjust to the filepath on your PC
t = data[:, 0]
x = data[:, 1]
y = data[:, 2]
t -= t[0]
